...
class ReminderViewTests(TestCase):
    ...
    def test_reminders_highlight_overdue_and_due_within_30_days(self):
        ...
        self.assertContains(response, f'<td class="row-overdue">{overdue.due_date}</td>', html=True)
        self.assertContains(response, f'<td class="row-warning">{warning.due_date}</td>', html=True)
